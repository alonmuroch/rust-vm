...
fn run_single_test(elf_path: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (code, code_start) = elf
        .get_flat_code()
        .ok_or("No code section in ELF")?;
    let (rodata, rodata_start) = elf
        .get_flat_rodata()
        .unwrap_or((vec![], usize::MAX as u64));
    ...
    let host: Box<dyn vm::host_interface::HostInterface> =
        Box::new(vm::host_interface::NoopHost {});
    ...
    let mut vm = VM::new_with_syscall_handler(memory.clone(), storage, host, syscall_handler);
    ...
    if !rodata.is_empty() {
        println!(
            "Writing rodata to memory: addr=0x{:x}, size=0x{:x}",
            rodata_start, rodata.len()
        );
        memory.write_code(rodata_start as usize, &rodata);
    }

    if !data.is_empty() {
        println!(
            "Writing data to memory: addr=0x{:x}, size=0x{:x}",
            data_start, data.len()
        );
        memory.write_code(data_start as usize, &data);
    }
...
fn test_riscv_spec() {
    ...
    if total_failed == 0 {
        ...
        panic!("❌ Some tests failed. Please review the results above.");
    }
}

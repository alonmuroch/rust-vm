...
use super::{CPU, Instruction, MemoryAccessKind, MemoryPage};
    ...
    pub fn execute(
        ...
        match instr {
            ...
            Instruction::Add { rd, rs1, rs2 } => {
                ...
                let lhs = match self.read_reg(rs1) { Some(v) => v, None => return false };
                let rhs = match self.read_reg(rs2) { Some(v) => v, None => return false };
                if !self.write_reg(rd, lhs.wrapping_add(rhs)) { return false; }
            }
            Instruction::Sub { rd, rs1, rs2 } => {
                let lhs = match self.read_reg(rs1) { Some(v) => v, None => return false };
                let rhs = match self.read_reg(rs2) { Some(v) => v, None => return false };
                if !self.write_reg(rd, lhs.wrapping_sub(rhs)) { return false; }
            }
            Instruction::Addi { rd, rs1, imm } => {
                let lhs = match self.read_reg(rs1) { Some(v) => v, None => return false };
                if !self.write_reg(rd, lhs.wrapping_add(imm as u32)) { return false; }
            }
            ...
            Instruction::And { rd, rs1, rs2 } => {
                let lhs = match self.read_reg(rs1) { Some(v) => v, None => return false };
                let rhs = match self.read_reg(rs2) { Some(v) => v, None => return false };
                if !self.write_reg(rd, lhs & rhs) { return false; }
            }
            ...
            Instruction::Andi { rd, rs1, imm } => {
                let lhs = match self.read_reg(rs1) { Some(v) => v, None => return false };
                if !self.write_reg(rd, lhs & (imm as u32)) { return false; }
            }
            ...
            Instruction::Slt { rd, rs1, rs2 } => {
                ...
                let lhs = match self.read_reg(rs1) { Some(v) => v, None => return false };
                let rhs = match self.read_reg(rs2) { Some(v) => v, None => return false };
                if !self.write_reg(rd, (lhs as i32).lt(&(rhs as i32)) as u32) { return false; }
            }
            Instruction::Sltu { rd, rs1, rs2 } => {
                // EDUCATIONAL: Set if less than (unsigned comparison)
                let lhs = match self.read_reg(rs1) { Some(v) => v, None => return false };
                let rhs = match self.read_reg(rs2) { Some(v) => v, None => return false };
                if !self.write_reg(rd, (lhs < rhs) as u32) { return false; }
            }
            Instruction::Slti { rd, rs1, imm } => {
                let lhs = match self.read_reg(rs1) { Some(v) => v, None => return false };
                if !self.write_reg(rd, (lhs as i32).lt(&imm) as u32) { return false; }
            }
            Instruction::Sltiu { rd, rs1, imm } => {
                let lhs = match self.read_reg(rs1) { Some(v) => v, None => return false };
                let rhs = imm as u32;
                if !self.write_reg(rd, if lhs < rhs { 1 } else { 0 }) { return false; }
            }
            ...
            Instruction::Sll { rd, rs1, rs2 } => {
                ...
                let lhs = match self.read_reg(rs1) { Some(v) => v, None => return false };
                let rhs = match self.read_reg(rs2) { Some(v) => v, None => return false };
                if !self.write_reg(rd, lhs << (rhs & 0x1F)) { return false; }
            }
            ...
            Instruction::Sra { rd, rs1, rs2 } => {
                ...
                let lhs = match self.read_reg(rs1) { Some(v) => v, None => return false };
                let rhs = match self.read_reg(rs2) { Some(v) => v, None => return false };
                if !self.write_reg(rd, ((lhs as i32) >> (rhs & 0x1F)) as u32) { return false; }
            }
            Instruction::Slli { rd, rs1, shamt } => {
                let lhs = match self.read_reg(rs1) { Some(v) => v, None => return false };
                if !self.write_reg(rd, lhs << shamt) { return false; }
            }
            ...
            Instruction::Srai { rd, rs1, shamt } => {
                let lhs = match self.read_reg(rs1) { Some(v) => v, None => return false };
                if !self.write_reg(rd, ((lhs as i32) >> shamt) as u32) { return false; }
            }
            ...
            Instruction::Lw { rd, rs1, offset } => {
                ...
                let base = match self.read_reg(rs1) { Some(v) => v, None => return false };
                let addr = base.wrapping_add(offset as u32) as usize;
                let val = match memory.borrow().load_u32(addr, self.metering.as_mut(), MemoryAccessKind::Load) {
                    Some(v) => v,
                    None => return false,
                };
                if !self.write_reg(rd, val) { return false; }
            }
            ...
            Instruction::Lb { rd, rs1, offset } => {
                ...
                let base = match self.read_reg(rs1) { Some(v) => v, None => return false };
                let addr = base.wrapping_add(offset as u32) as usize;
                let byte = match memory.borrow().load_byte(addr, self.metering.as_mut(), MemoryAccessKind::Load) {
                    Some(v) => v,
                    None => return false,
                };
                let value = (byte as i8) as i32 as u32; // sign-extend to 32-bit
                if !self.write_reg(rd, value) { return false; }
            }
            Instruction::Lbu { rd, rs1, offset } => {
                ...
                if !self.write_reg(rd, byte as u32) { return false; }
            }
            Instruction::Lh { rd, rs1, offset } => {
                // EDUCATIONAL: Load halfword (16-bit, sign-extended)
                let base = match self.read_reg(rs1) { Some(v) => v, None => return false };
                let addr = base.wrapping_add(offset as u32) as usize;
                let halfword = match memory.borrow().load_halfword(addr, self.metering.as_mut(), MemoryAccessKind::Load) {
                    Some(v) => v,
                    None => return false,
                };
            ...
            Instruction::Lhu { rd, rs1, offset } => {
                ...
                if !self.write_reg(rd, halfword as u32) { return false; } // zero-extend to 32-bit
            }
            ...
            Instruction::Sh { rs1, rs2, offset } => {
                ...
                let base = match self.read_reg(rs1) { Some(v) => v, None => return false };
                let addr = base.wrapping_add(offset as u32) as usize;
                let src = match self.read_reg(rs2) { Some(v) => v, None => return false };
                if !memory.borrow().store_u16(addr, (src & 0xFFFF) as u16, self.metering.as_mut(), MemoryAccessKind::Store) {
                    return false;
                }
            }
            Instruction::Sw { rs1, rs2, offset } => {
                ...
                let addr = base.wrapping_add(offset as u32) as usize;
                let src = match self.read_reg(rs2) { Some(v) => v, None => return false };
                if !memory.borrow().store_u32(addr, src, self.metering.as_mut(), MemoryAccessKind::Store) {
                    return false;
                }
            }
            Instruction::Sb { rs1, rs2, offset } => {
                ...
                let addr = base.wrapping_add(offset as u32) as usize;
                let src = match self.read_reg(rs2) { Some(v) => v, None => return false };
                if !memory.borrow().store_u8(addr, (src & 0xFF) as u8, self.metering.as_mut(), MemoryAccessKind::Store) {
                    return false;
                }
            }
            ...
            Instruction::Beq { rs1, rs2, offset } => {
                ...
                let rhs = match self.read_reg(rs2) { Some(v) => v, None => return false };
                if lhs == rhs {
                    if !self.pc_add(offset as u32) { return false; }
                    return true;
                }
            }
            ...
            Instruction::Jal { rd, offset, compressed } => {
                ...
                if !self.write_reg(rd, return_address) { return false; }
                if !self.pc_add(offset as u32) { return false; }
                return true;
            }
            Instruction::Jalr { rd, rs1, offset , compressed} => {
                ...
                if !self.set_pc(target) { return false; }
                return true;
            }
            ...
            Instruction::Lui { rd, imm } => {
                ...
                if !self.write_reg(rd, (imm << 12) as u32) { return false; }
            }
            Instruction::Auipc { rd, imm } => {
                ...
                if !self.write_reg(rd, self.pc.wrapping_add((imm << 12) as u32)) { return false; }
            }
            ...
            Instruction::Mul { rd, rs1, rs2 } => {
                ...
                let lhs = match self.read_reg(rs1) { Some(v) => v, None => return false };
                let rhs = match self.read_reg(rs2) { Some(v) => v, None => return false };
                if !self.write_reg(rd, lhs.wrapping_mul(rhs)) { return false; }
            }
            Instruction::Mulh { rd, rs1, rs2 } => {
                ...
                let val1 = (match self.read_reg(rs1) { Some(v) => v, None => return false } as i32) as i64;
                let val2 = (match self.read_reg(rs2) { Some(v) => v, None => return false } as i32) as i64;
                let result = val1 * val2;
                if !self.write_reg(rd, (result >> 32) as u32) { return false; }
            }
            Instruction::Mulhu { rd, rs1, rs2 } => {
                // EDUCATIONAL: MULHU - multiply unsigned, store upper 32 bits
                let lhs = match self.read_reg(rs1) { Some(v) => v as u64, None => return false };
                let rhs = match self.read_reg(rs2) { Some(v) => v as u64, None => return false };
                if !self.write_reg(rd, ((lhs * rhs) >> 32) as u32) { return false; }
            }
            Instruction::Mulhsu { rd, rs1, rs2 } => {
                ...
                let val1 = (match self.read_reg(rs1) { Some(v) => v, None => return false } as i32) as i64;
                let val2 = match self.read_reg(rs2) { Some(v) => v as u64, None => return false };
            ...
            Instruction::Div { rd, rs1, rs2 } => {
                ...
                let rhs = match self.read_reg(rs2) { Some(v) => v, None => return false };
                if rhs == 0 {
                    if !self.write_reg(rd, 0xFFFFFFFF) { return false; } // -1 in two's complement
                } else {
                    ...
                    if dividend == i32::MIN && divisor == -1 {
                        if !self.write_reg(rd, lhs) { return false; } // Return dividend on overflow
                    } else {
                        if !self.write_reg(rd, (dividend / divisor) as u32) { return false; }
                    }
                }
            }
            Instruction::Divu { rd, rs1, rs2 } => {
                ...
                let rhs = match self.read_reg(rs2) { Some(v) => v, None => return false };
                if rhs == 0 {
                    if !self.write_reg(rd, 0xFFFFFFFF) { return false; } // 2^32 - 1
                } else {
                    if !self.write_reg(rd, lhs / rhs) { return false; }
                }
            }
            Instruction::Rem { rd, rs1, rs2 } => {
                ...
                let rhs = match self.read_reg(rs2) { Some(v) => v, None => return false };
                if rhs == 0 {
                    if !self.write_reg(rd, lhs) { return false; }
                } else {
                    let dividend = lhs as i32;
                    let divisor = rhs as i32;
                    ...
                    if dividend == i32::MIN && divisor == -1 {
                        if !self.write_reg(rd, 0) { return false; } // Remainder of -2^31 % -1 is 0
                    } else {
                        if !self.write_reg(rd, (dividend % divisor) as u32) { return false; }
                    }
                }
            ...
            Instruction::Ecall => {
                ...
                let args = [
                    match self.read_reg(Register::A1 as usize) { Some(v) => v, None => return false },
                    match self.read_reg(Register::A2 as usize) { Some(v) => v, None => return false },
                    match self.read_reg(Register::A3 as usize) { Some(v) => v, None => return false },
                    match self.read_reg(Register::A4 as usize) { Some(v) => v, None => return false },
                    match self.read_reg(Register::A5 as usize) { Some(v) => v, None => return false },
                    match self.read_reg(Register::A6 as usize) { Some(v) => v, None => return false },
                ];
                let call_id = match self.read_reg(Register::A7 as usize) { Some(v) => v, None => return false };
                ...
                if !self.write_reg(Register::A0 as usize, result) { return false; }
                return cont;
            }
            Instruction::Csr { rd, rs1, csr, op, imm } => {
                let src = if imm { rs1 as u32 } else { match self.read_reg(rs1) { Some(v) => v, None => return false } };
                let old = match self.read_csr(csr) { Some(v) => v, None => return false };
                ...
                if src != 0 || matches!(op, CsrOp::Csrrw) {
                    if !self.write_csr(csr, new_val) { return false; }
                }

                if rd != 0 && !self.write_reg(rd, old) { return false; }
            }
            Instruction::Ebreak => {
            ...
            Instruction::Jr { rs1 } => {
                ...
                let target = match self.read_reg(rs1) { Some(v) => v, None => return false };
                if !self.set_pc(target) { return false; }
                return true;
            }
            Instruction::Ret => {
                ...
                let target = match self.read_reg(1) { Some(v) => v, None => return false }; // x1 = ra (return address)
            ...
            Instruction::Addi4spn { rd, imm } => {
                ...
                if !self.write_reg(rd, sp.wrapping_add(imm)) { return false; }
            }
            Instruction::Nop => {
            ...
            Instruction::Beqz { rs1, offset } => {
                ...
                let val = match self.read_reg(rs1) { Some(v) => v, None => return false };
                if val == 0 {
                    if !self.pc_add(offset as u32) { return false; }
                    return true;
                }
            ...
            Instruction::MiscAlu { rd, rs2, op } => {
                match op {
                    crate::instruction::MiscAluOp::Sub => {
                        ...
                        let lhs = match self.read_reg(rd) { Some(v) => v, None => return false };
                        let rhs = match self.read_reg(rs2) { Some(v) => v, None => return false };
                        if !self.write_reg(rd, lhs.wrapping_sub(rhs)) { return false; }
                    }
            ...
            Instruction::AmoswapW { rd, rs1, rs2 } => {
                ...
                let orig = match memory.borrow().load_u32(addr, self.metering.as_mut(), MemoryAccessKind::Atomic) {
                    Some(v) => v,
                    None => return false,
                };
                if !memory.borrow().store_u32(addr, src, self.metering.as_mut(), MemoryAccessKind::Atomic) {
                    return false;
                }
                if !self.write_reg(rd, orig) { return false; }
            }
            Instruction::AmoaddW { rd, rs1, rs2 } => {
                ...
                if !memory.borrow().store_u32(addr, new_val, self.metering.as_mut(), MemoryAccessKind::Atomic) {
                    return false;
                }
                if !self.write_reg(rd, orig) { return false; }
            }
            ...
            Instruction::AmomaxW { rd, rs1, rs2 } => {
                ...
                let new_val = if (orig as i32) > (src as i32) { orig } else { src };
                if !memory.borrow().store_u32(addr, new_val, self.metering.as_mut(), MemoryAccessKind::Atomic) {
                    return false;
                }
                if !self.write_reg(rd, orig) { return false; }
            ...
            Instruction::LrW { rd, rs1 } => {
                let base = match self.read_reg(rs1) { Some(v) => v, None => return false };
                let addr = base as usize;
                let value = match memory.borrow().load_u32(addr, self.metering.as_mut(), MemoryAccessKind::ReservationLoad) {
                    Some(v) => v,
                    None => return false,
                };
                if !self.write_reg(rd, value) { return false; }
            ...
            Instruction::ScW { rd, rs1, rs2 } => {
                let base = match self.read_reg(rs1) { Some(v) => v, None => return false };
                let addr = base as usize;
                let value_to_store = match self.read_reg(rs2) { Some(v) => v, None => return false };
                ...
                if self.reservation_addr == Some(addr) {
                    ...
                    if !memory.borrow().store_u32(addr, value_to_store, self.metering.as_mut(), MemoryAccessKind::ReservationStore) {
                        return false;
                    }
                    if !self.write_reg(rd, 0) { return false; } // 0 = success
                    // Clear the reservation (it's consumed)
                    self.reservation_addr = None;
                } else {
                    // No valid reservation, fail
                    if !self.write_reg(rd, 1) { return false; } // 1 = failure
                }
            }
